...
sim_events <- detection_events(sim_glatos, condense = F)
...
sim_basin_locf_full <- left_join(sim_locf_region,
                                 aggregate(deploy_lat~regions, data = sim_locf, FUN = mean)) %>% 
  rename("latitude" = deploy_lat)


str(sim_basin_locf_full)
###----------------------------------------------------------------------------------------------------


###----------------------------------------------------------------------------------------------------
### Model 3: Centers of Activity
###----------------------------------------------------------------------------------------------------
### Data preparation
# Load vTrack data
...
str(sim_basin_coa_full)
###----------------------------------------------------------------------------------------------------


###----------------------------------------------------------------------------------------------------
### Model 4: Standardized detections: GLATOS non-linear interpolated path
###----------------------------------------------------------------------------------------------------
# Load data
sim_glatos <- readRDS("outputs/ModelEstimates/sim_glatos.rds")
...
# calculate average receiver latitude
sim_rec_lat_glatos <- aggregate(latitude~regions, data = sim_int_df_full, FUN = mean)

# merge number of detections for each fish in each basin with total detections for each fish 
sim_basin_glatos_full <- left_join(sim_basin_glatos, sim_total_detect_glatos) %>% 
  left_join(sim_rec_lat_glatos)

# calculate percentage of detections in each basin for each fish
...
###----------------------------------------------------------------------------------------------------
### Model 5: move dynamic Brownian Bridge Movement Model
###----------------------------------------------------------------------------------------------------
### Load data
sim_dets_move <- readRDS("outputs/ModelEstimates/sim_move.rds")
...
simProj <- spTransform(sim_dets_move, CRSobj="+proj=utm +zone=18 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
...
sim_basin_move_full <- data.frame(dBBMM_lake) %>% 
...
###----------------------------------------------------------------------------------------------------
### Model 6: RSP dynamic Brownian Bridge Movement Model
###----------------------------------------------------------------------------------------------------
# ### load data
...
dBBMM_regions_ref <- dBBMM_regions_df %>% 
...
# calculate total time 
sim_rsp_occ.1 <- dBBMM_regions_ref %>%
...
# calculate region time and percent
sim_basin_rsp_full <- dBBMM_regions_ref %>% 
  mutate(prob = 1-value, # take inverse value for space use to get ~probability
         prob = if_else(prob == 1, 0, prob)) %>%  # remove cells that had values of 0
  group_by(animal_id, regions, .drop = F) %>% 
